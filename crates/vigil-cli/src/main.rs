...
enum Commands {
    ...
    /// Show the current effective pricing table (built-in defaults or ~/.vigil/pricing.toml)
    Pricing {
        /// Write built-in defaults to ~/.vigil/pricing.toml
        #[arg(long)]
        write: bool,
        /// Overwrite an existing pricing.toml (only with --write)
        #[arg(long)]
        force: bool,
    },
}
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Pricing { write, force }) => {
            run_pricing(write, force)?;
        }
    }
...
async fn vigil_init(output: PathBuf, force: bool) -> Result<()> {
    ...
    // Also generate vigil.toml with all sections commented out.
    let toml_path = PathBuf::from("vigil.toml");
    if !toml_path.exists() || force {
        let vigil_toml = generate_vigil_toml();
        std::fs::write(&toml_path, &vigil_toml)?;
        println!("Created vigil.toml — uncomment and adjust sections as needed.");
        println!("Run with: vigil run --config vigil.toml -- <agent>");
    } else {
        println!("vigil.toml already exists — skipping (use --force to overwrite).");
    }

    Ok(())
}

fn generate_vigil_toml() -> String {
    r#"# vigil.toml — generated by `vigil init`
# Uncomment and adjust sections as needed.
# Run with: vigil run --config vigil.toml -- <agent>

[proxy]
# port = 8877
# write_approval_threshold = "Medium"       # "Low" | "Medium" | "High"
# tool_timeout_secs = 600                   # alert after N seconds of tool silence
# tool_timeout_kill_secs = 900              # kill agent after N seconds (>= tool_timeout_secs)

[web]
# port = 8878                               # browser dashboard; omit to disable

[budget]
# max_cost_usd = 5.0                        # hard stop at this spend
# max_tokens = 500000                       # hard stop at this token count
# cost_alert_usd = 2.0                      # soft alert (no stop) at this spend
# max_burn_rate_usd_per_min = 0.10          # BURN alert when rolling $/min exceeds this
# max_session_duration_mins = 60            # DURA alert after this many minutes
# max_sub_agent_depth = 3                   # deny Task tool calls beyond this depth
# allowed_hours = "09:00-18:00"            # only allow runs during this window (local time)

[session]
# store_raw = false                         # store raw request/response bodies

[pii]
# watchlist_file = ".vigil-watchlist"       # one custom term per line

[notify]
# webhook = "https://hooks.example.com/..."
# webhook_events = ["BURN", "EXFL", "DENY"] # omit to forward all alerts
...
fn run_pricing(write: bool, force: bool) -> Result<()> {
    let home = if cfg!(target_os = "windows") {
        std::env::var("USERPROFILE").ok()
    } else {
        std::env::var("HOME").ok()
    ...
    let pricing_path = PathBuf::from(&home).join(".vigil").join("pricing.toml");

    let (table, source) = if pricing_path.exists() {
        let content = std::fs::read_to_string(&pricing_path)?;
        match toml::from_str::<vigil_core::pricing::PricingTable>(&content) {
            Ok(t) => (t, format!("{}", pricing_path.display())),
            Err(e) => {
                eprintln!("Warning: {} is invalid ({}); using built-in defaults.", pricing_path.display(), e);
                (vigil_core::PricingTable::defaults(), "built-in defaults".to_string())
            }
        }
    } else {
        (vigil_core::PricingTable::defaults(), "built-in defaults".to_string())
    ...
    println!("Effective pricing table ({}):\n", source);
    println!("{:<35}  {:>12}  {:>12}", "Pattern", "Input/M USD", "Output/M USD");
    println!("{}", "-".repeat(63));
    for m in &table.models {
        println!(
            "{:<35}  {:>12}  {:>12}",
            m.pattern,
            format!("${:.4}", m.input_per_million),
            format!("${:.4}", m.output_per_million),
        );
    }

    if write {
        if pricing_path.exists() && !force {
            anyhow::bail!(
                "'{}' already exists. Use --force to overwrite.",
                pricing_path.display()
            );
        }
        let defaults = vigil_core::PricingTable::defaults();
        let toml_str = toml::to_string_pretty(&defaults)
            .context("Failed to serialize pricing table")?;
        if let Some(parent) = pricing_path.parent() {
            std::fs::create_dir_all(parent)?;
        }
        std::fs::write(&pricing_path, toml_str)?;
        println!("\nWrote defaults to {}", pricing_path.display());
        println!("Edit the file and restart vigil to apply custom pricing.");
    }
